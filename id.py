	...
	for i in range(iteration_depth):
		while(len(open_stack) > 0):
			...
			if(current_node.state == solution_node):
				...
				print("Depth of Iteration: " + str(i))
				print("*************************************************")
			...
			while(current_node.depth < max_depth):
				...
				# when trying to pop an empty stack at the end of the search
				# CAUSE: when a node generates a set of children
				# 		 that have all been visited, i.e all in the closed stack
				if(len(open_stack) == 0):
					break
				else:
					current_node = open_stack.pop()
...
def output_solution_path(node):
	new_file = open("output_files/id/solution_path.txt", "w+")
...
def output_search_path(closed_stack):
	new_file = open("output_files/id/search_path.txt", "w+")
